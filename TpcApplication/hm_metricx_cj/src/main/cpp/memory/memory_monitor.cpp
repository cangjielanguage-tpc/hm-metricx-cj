...
HOOK(void, free, void *ptr) {
  free(ptr);
  if (ptr) {
    MemoryMonitor::GetInstance().UnregisterAlloc(
...
HOOK(void *,munmap,void *addr, size_t size, int prot, int flags, int fd, off_t offset){
  auto result =mummap(addr,size,prot,flags,fd,offset);
  MemoryMonitor::GetInstance().OnMonitor(reinterpret_cast<intptr_t>(result),size);
  CLEAR_MEMORY(result, size);
  return result;
}
...
HOOK(void *, malloc, size_t size) {
  ...
  MemoryMonitor::GetInstance().OnMonitor(reinterpret_cast<intptr_t>(result),
                                       size);
  CLEAR_MEMORY(result, size);
  return result;
}
...
HOOK(int, posix_memalign, void **memptr, size_t alignment, size_t size) {
  ...
  MemoryMonitor::GetInstance().OnMonitor(reinterpret_cast<intptr_t>(*memptr),
                                       size);
...
void MemoryMonitor::SaveAllocRecordsToFile(const std::string &filename){
  if(live_alloc_records_.Size()==0){
    OH_LOG_Print(LOG_APP,LOG_WARN,0x00008,"MemoryMonitor",
  ...
  if(!ofs.is_open()){
    OH_LOG_Print(LOG_APP,LOG_WARN,0x00008,"MemoryMonitor",
      "MemoryMonitor::DumpLiveAllocRecordsToFile - Failed to open file %s for writeing.",filename.c_str());
      return;
    }
  size_t count=0;
  auto predicate=[this,&ofs,&count](const std::shared_ptr<AllocRecord> &record){
     try{
      ...
      OH_LOG_Print(LOG_APP,LOG_WARN,0x00008,"MemoryMonitor",
        "MemoryMonitor::DumpLiveAllocRecordsToFile - Error writing record %{public}zu %{public}s.",count+1,e.what());
        return false;
     }
  };
...
bool MemoryMonitor::Install(std::vector<std::string> *selected_list,
  ...
  if (has_install_monitor_) {
    OH_LOG_Print(LOG_APP,LOG_WARN,0x00008,"MemoryMonitor",
      "MemoryMonitor::Install - already installed");
    return true;
  }
  ...
  std::vector<std::pair<const std::string, void *const>> hook_entries = {
      ...
      std::make_pair("mmap",
      reinterpret_cast<void *>(WRAP(mmap))),
      std::make_pair("munmap",
        reinterpret_cast<void *>(WRAP(munmap))),
  ...
  OH_LOG_Print(LOG_APP,LOG_WARN,0x00008,"MemoryMonitor",
    "MemoryMonitor::Install - install failed");
...
void MemoryMonitor::SetMonitorThreshold(size_t threshold) {
...
ALWAYS_INLINE void MemoryMonitor::UnregisterAlloc(uintptr_t address) {
