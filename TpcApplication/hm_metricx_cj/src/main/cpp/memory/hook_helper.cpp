...
void *(*dlopen_ext_origin)(const char *file,int mode,const char *space,
                          const void *caller_addr, const dl_extinfo *extinfo);

void *dlopen_hook(const char *file,int mode,const char *space,
const void *caller_addr, const dl_extinfo *extinfo){
...
bool HookHelper::HookMethods(
  ...
  if (register_pattern.empty() || methods.empty()) {
    OH_LOG_Print(LOG_APP,LOG_WARN,0x00008,"MemoryMonitor","HookHelper::HookMethods - Nothing to hook");
    return false;
  }
...
bool HookHelper::HookImpl() {
  ...
  xhook_clear();
  if(xhook_register("ld-musl-aarch64.so.1","dlopen_impl",(void *)dlopen_hook,(void **)(&dlopen_ext_origin))!=EXIT_SUCCESS){
      OH_LOG_Print(LOG_APP,LOG_WARN,0x00008,"MemoryMonitor","HookHelper::HookImpl - register dlopen_impl failed");
      return false;
  }
  ...
  for (auto &pattern : ignore_pattern_) {
    for (auto &method : methods_) {
      if (xhook_ignore(pattern.c_str(), method.first.c_str()) != EXIT_SUCCESS) {
        OH_LOG_Print(LOG_APP,LOG_WARN,0x00008,"MemoryMonitor",
          "HookHelper::HookImpl - Failed to ignore pattern: %s,method: %s",
          pattern.c_str(),method.first.c_str());
        pthread_mutex_unlock(&hook_mutex);
        return false;
      }
